#!/bin/bash
# Buduje KDE Plasma Desktop — pełne środowisko z paskiem, tapetą, kwin
# Kolejność: deps → kwin → plasma-workspace → plasma-desktop
# Użycie: cd ~/warppkgs && sudo bash scripts/plasma-build.sh
# Szacowany czas: ~15-20 SBU
set -uo pipefail
export MAKEFLAGS="-j1"
...
# ============================================================
# Fix: plasma-activities — wyłącz Boost (opcjonalny)
# ============================================================
cat > "$SCRIPTDIR/desktop/plasma-activities/WARPBUILD" << 'EOF'
...
# ============================================================
# Fix: kwin — wyłącz opcjonalne deps których nie mamy
# ============================================================
...
# ============================================================
# Fix: plasma-workspace — wyłącz opcjonalne deps
# ============================================================
...
# ============================================================
# Fix: plasma-desktop — wyłącz opcjonalne deps
# ============================================================
...
echo -e "\n${YELLOW}=== Etap 1: Brakujące frameworki ===${NC}\n"

build_pkg desktop plasma-activities
build_pkg desktop baloo
build_pkg desktop plasma5support

echo -e "\n${YELLOW}=== Etap 2: Wayland/display deps ===${NC}\n"

build_pkg desktop layer-shell-qt
build_pkg desktop libkscreen
build_pkg desktop kglobalacceld

echo -e "\n${YELLOW}=== Etap 3: Plasma core ===${NC}\n"

build_pkg desktop libplasma
build_pkg desktop kscreenlocker

echo -e "\n${YELLOW}=== Etap 4: KWin (window manager) ===${NC}\n"

build_pkg desktop kwin

echo -e "\n${YELLOW}=== Etap 5: Plasma workspace + desktop ===${NC}\n"
